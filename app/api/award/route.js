...
export async function GET(req) {
  try {
    const categories = await prisma.award.findMany();
...
export async function POST(req) {
  try {
    ...
    const category = await prisma.award.create({ data: { img } });
...
export async function PATCH(req) {
  ...
  try {
    ...
    const updatedCategory = await prisma.award.update({
      where: { id },
      data: { img  },
    });
...
export async function DELETE(req) {
  try {
    ...
    const deletedCategory = await prisma.award.delete({ where: { id } });
