...
const ManageCategory = () => {
  ...
  const fetchCategories = async () => {
    try {
      const res = await fetch('/api/video', { method: 'GET' });
      if (res.ok) {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`/api/video?id=${encodeURIComponent(editFormData.id)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">{editMode ? 'Edit video' : 'Add video'}</h1>
      <form onSubmit={editMode ? handleEditSubmit : handleSubmit} className="space-y-4">
         
       
        <Upload1 onFilesUpload={handleImgChange} />
      ...
      <h2 className="text-xl font-bold mt-8">All videos</h2>
